 ...
 *
 *           (Most of this code was adapted from textbooks e.g. by Frank
 *           Carrano, Mark Allen Weiss, Michael Goodrich and Roberto Tomassia)
 *
 *           I put some Javadoc in this code, e.g. @param, @return But I do not
 *           expect you to do so in this course (and I generally don't do it
 *           when I am writing my own Java code)
 *
 */
...
public class SLinkedList<E> implements Iterable<E> {
    ...
    /**
     * Inserts the element to the specified position in this list where index is
     * from 0 to size. If the position is size, then add element to the end of the
     * list.
     *
    ...
    /*
     * TODO: Makes a deep copy of the this linked list. Hint: Use E.deepCopy().
     */
    public SLinkedList<E> deepClone() {
        SLinkedList<E> clonedList = new SLinkedList<E>();
        for (E node : this) {
            clonedList.addLast(node.deepClone());
        }
        return clonedList;
    }

    // The following method is private. The client has no access to the nodes of the
    // linked list,
    // but rather the client can only access the elements that are stored in the
    // list.
    /**
    ...
    private class SLLIterator implements Iterator<E> { // use a different generic type, since we're defining a new class
        // here
        SNode<E> cur;
